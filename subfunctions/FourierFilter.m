...
[xx yy]=meshgrid(-(xw-1)/2:(xw-1)/2,-(yw-1)/2:(yw-1)/2);
    ...
    AA=whos('Imagein');
    try totalmax=intmax(AA.class); end; %#ok<TRYNC>
    %if AA.class==deal('logical'); totalmax=1; end;
...
%         'Scalefactor:' num2str(scalefactor);'Rangevalue:' num2str(valuerange)})
    ...
    edgemask=im2bw(Imagein(:,:,channel,1),scalefactor+0.1*valuerange);
    %edgemask=imclose(edgemask,strel('disk',10));
...
%       imagesc(edgemask)
    ...
    IFFTIMAGE=real(ifft2(FilteredFourier));
    %IFFTIMAGE=IFFTIMAGE-min(min(IFFTIMAGE));
    if edgefilter>=0; imageout=immultiply(IFFTIMAGE,edgemask); end;
    if edgefilter<0; imageout=IFFTIMAGE; end;
